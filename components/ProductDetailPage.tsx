          ...
          <nav className="flex items-center gap-1.5 text-sm text-gray-500 flex-wrap">
            {["Home", typeof product.category === "string" ? product.category : product.category?.name || "Products", product.name.split(" ").slice(0, 3).join(" ")].map((crumb, i, arr) => (
              <span key={crumb} className="flex items-center gap-1.5">
                <button
                  onClick={i === 0 ? onBack : i === 1 ? onShopClick : undefined}
                  className={
                    i < arr.length - 1
                      ? "hover:text-[#2c1654] transition-colors font-medium cursor-pointer"
                      : "text-gray-900 font-black pointer-events-none truncate max-w-[140px]"
                  }
                >
                  {crumb}
                </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 mb-14">
                ...
                {images.map((img, i) => (
                  <button
                    ...
                    className={`aspect-square rounded-xl overflow-hidden border-2 transition-all ${
                      activeImage === i ? "ring-2" : "border-gray-200 hover:border-gray-400"
                    }`}
                    style={activeImage === i ? { borderColor: "#2c1654" } : {}}
                  >
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm leading-relaxed mb-5 pb-5 border-b border-gray-100">
              {product.description ||
                "Experience this premium product, designed to offer high comfort and durability in everyday use."}
            </p>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {product.colors.map((color) => (
                    <button
                      ...
                      style={
                        activeColor === color
                          ? { borderColor: "#2c1654", backgroundColor: "#f5f0ff", color: "#2c1654" }
            ...
            <div className="flex gap-3 mb-5 mt-6">
              ...
              <button
                onClick={() => onCheckout(product, activeColor, activeSize)}
                className="flex-1 font-black py-4 rounded-2xl transition-opacity hover:opacity-90 text-white text-sm"
            ...
            <div className="border rounded-2xl p-4 space-y-3" style={{ borderColor: "#e8e5f0", backgroundColor: "#fbfaff" }}>
              {[
                ...
                { icon: Truck, text: <><span className="font-bold text-emerald-700">Free shipping</span> over ৳5,000</> },
                { icon: RotateCcw, text: <>30-day easy returns · <a href="#" className="font-bold" style={{ color: "#2c1654" }}>Learn more</a></> },
          ...
          <div className="flex border-b border-gray-200 overflow-x-auto scrollbar-hide">
            {tabs.map((tab) => (
              <button
                ...
                className="px-6 py-4 text-sm font-black whitespace-nowrap border-b-2 transition-colors"
                style={
                  activeTab === tab
                    ? { borderColor: "#2c1654", color: "#2c1654" }
          ...
          <div className="pt-8">
            {activeTab === "Description" && (
              <div className="max-w-2xl space-y-4 text-gray-600 text-sm leading-relaxed">
                <p>
                  {product.description ||
                    "This high-performance item is crafted to satisfy strict quality control standards. Offering sleek styling and functional attributes that elevate your modern daily routine."}
                </p>
