...
import {
  Search, ShoppingCart, Heart, MapPin, Globe, User, Package, Menu, X
} from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3.5">
        <div className="flex items-center gap-3 lg:gap-5">
          ...
          <div className="flex-1 flex items-center bg-[#f8f7fc] border border-purple-100 rounded-xl overflow-hidden focus-within:border-[#2c1654] focus-within:ring-2 focus-within:ring-purple-100 transition-all">
            ...
            <button
              onClick={onShopClick}
              className="text-white px-5 py-2.5 text-sm font-bold transition-opacity hover:opacity-90"
          ...
          <div className="hidden md:flex items-center gap-0.5">
            {[
              ...
              <button
                key={label}
                onClick={action}
                className="flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl hover:bg-purple-50 transition-colors group"
              >
                <Icon className="w-5 h-5 text-gray-500 group-hover:text-[#2c1654] transition-colors" />
                <span className="text-[10px] text-gray-500 group-hover:text-[#2c1654] leading-none transition-colors">
                  {label}
                </span>
              </button>
            ))}
            <button
              onClick={onCartOpen}
              className="relative flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl hover:bg-purple-50 transition-colors group"
            >
              ...
              {cartCount > 0 && (
                <span
                  className="absolute top-1 right-1.5 w-4 h-4 text-white text-[9px] font-black rounded-full flex items-center justify-center"
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white px-4 py-3 space-y-1">
          {[
            ...
            <button
              key={label}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-purple-50 text-sm font-semibold text-gray-700 transition-colors"
            >
