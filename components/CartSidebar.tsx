      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 bg-[#2c1654]">
          <div className="flex items-center gap-3">
            ...
            <span className="bg-[#c8960c] text-white text-xs font-black px-2 py-0.5 rounded-full">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <button
                onClick={onClose}
                className="mt-5 bg-[#2c1654] text-white font-bold px-6 py-2.5 rounded-xl text-sm hover:opacity-90 transition-opacity"
              >
                Continue Shopping
              </button>
            </div>
            ...
            items.map((item, idx) => (
              <div key={idx} className="flex gap-4 bg-gray-50 rounded-2xl p-3.5">
                <div className="w-20 h-20 bg-white rounded-xl overflow-hidden shrink-0 border border-gray-100 relative">
                  <img
                    src={item.product.thumbnail || item.product.image || IMGS.headphones}
                    alt={item.product.name}
                    className="w-full h-full object-cover"
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-[#2c1654] uppercase tracking-wide mb-0.5">
                    {item.product.brand}
                  </p>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-white">
                      <button
                        onClick={() =>
                          onUpdateQty(item.product._id, item.color, item.size, item.quantity - 1)
                        }
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => onRemove(item.product._id, item.color, item.size)}
                  ...
                  <button
                    ...
                    className="w-full mt-2.5 py-1.5 text-xs font-bold text-white bg-[#c8960c] rounded-lg hover:opacity-90 transition-opacity"
                  >
                    Buy This Item
                  </button>
                </div>
        ...
        {items.length > 0 && (
          <div className="px-6 py-5 border-t border-gray-100 space-y-3">
            ...
            <button
              onClick={() => onCheckout(items[0])}
              className="w-full bg-[#2c1654] hover:opacity-90 text-white font-black py-4 rounded-2xl transition-opacity text-base shadow-xl"
            >
              Proceed to Checkout
            </button>
            <button
              onClick={onClose}
              className="w-full border-2 border-[#2c1654] text-[#2c1654] font-bold py-3 rounded-2xl text-sm hover:bg-[#f5f0ff] transition-colors"
            >
              Continue Shopping
            </button>
          </div>
