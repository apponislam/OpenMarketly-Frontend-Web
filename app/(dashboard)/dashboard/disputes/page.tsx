...
export default function DisputesPage() {
    ...
    return (
        <div className="space-y-8 max-w-7xl mx-auto font-sans">
            <div>
                <h1 className="text-2xl font-bold tracking-tight text-gray-900 md:text-3xl">Platform Disputes</h1>
                <p className="mt-1.5 text-sm text-gray-500">
                    {isAdmin ? "Manage raised order disputes and customer support requests." : "Track customer disputes raised on your products."}
                </p>
            </div>

            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                    <AlertOctagon className="h-5 w-5 text-[#2c1654]" /> Active Disputes Queue
                </h2>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {disputes.map((dispute) => (
                                <tr key={dispute._id} className="hover:bg-gray-50/50 transition-colors">
                                    <td className="px-4 py-3.5 font-bold text-[#2c1654]">
                                        {dispute.orderId || "N/A"}
                                    </td>
                                        ...
                                        {isAdmin && dispute.status === "PENDING" ? (
                                            <div className="flex flex-col gap-1.5 max-w-[220px]">
                                                <input
                                                    ...
                                                    onChange={(e) => setRemarks({ ...remarks, [dispute._id]: e.target.value })}
                                                    className="px-2.5 py-1 border border-gray-200 rounded-lg text-xs"
                                                />
                                                <div className="flex gap-2">
                                                    <button
                                                        onClick={() => handleResolve(dispute._id, "RESOLVED")}
                                            ...
                                            <span className="text-xs text-gray-400 italic">
                                                {dispute.adminRemarks || "No resolution details"}
                                            </span>
                                        )}
                                    </td>
                                </tr>
