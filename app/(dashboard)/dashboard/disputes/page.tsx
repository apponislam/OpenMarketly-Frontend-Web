...
export default function DisputesPage() {
    ...
    const handleResolve = async (id: string, status: "RESOLVED" | "REJECTED") => {
        try {
            ...
            refetchAdmin();
        } catch (err: any) { alert(err?.data?.message || "Failed to resolve dispute."); }
    };
    ...
    return (
        <div className="space-y-8 max-w-7xl mx-auto font-sans">
            <DashboardPageHeader
                ...
                subtitle={isAdmin ? "Manage raised order disputes and customer support requests." : "Track customer disputes raised on your products."}
            />
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {disputes.map((dispute) => (
                                <tr key={dispute._id} className="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="px-4 py-3.5"><StatusBadge status={dispute.status} /></td>
                                        ...
                                        {isAdmin && dispute.status === "PENDING" ? (
                                            <div className="flex flex-col gap-1.5 max-w-[220px]">
                                                ...
                                                <div className="flex gap-2">
                                                    <button onClick={() => handleResolve(dispute._id, "RESOLVED")} disabled={isResolving} className="px-2.5 py-1 bg-emerald-600 text-white rounded text-xs font-bold hover:bg-emerald-700 cursor-pointer">Mark Resolved</button>
                                                    <button onClick={() => handleResolve(dispute._id, "REJECTED")} disabled={isResolving} className="px-2.5 py-1 bg-red-600 text-white rounded text-xs font-bold hover:bg-red-700 cursor-pointer">Dismiss</button>
                                                </div>
                                            </div>
                                        ) : (
                            ...
                            {disputes.length === 0 && (
                                <tr><td colSpan={5} className="px-4 py-6 text-center text-sm text-gray-400">No active disputes found.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
