...
export default function Home() {
  ...
  const handleUpdateQty = (product_Id: string, color: string, size: string, newQty: number) => {
    ...
    setCart((prev) =>
      prev.map((item) =>
        item.product._id === product_Id && item.color === color && item.size === size
  ...
  const handleRemoveFromCart = (product_Id: string, color: string, size: string) => {
    setCart((prev) =>
      prev.filter((item) => !(item.product._id === product_Id && item.color === color && item.size === size))
    );
  };
    ...
    <div className="min-h-screen bg-white text-[#0d0a1a] flex flex-col">
      ...
      {page === "home" && (
        <HomePage
          ...
          onAddToCart={(p) => handleAddToCart(p, "Default", "Default")}
      ...
      {page === "product" && selectedProduct && (
        <ProductDetailPage
          ...
          onCheckout={handleOpenDirectCheckout}
      ...
      <CartSidebar
        ...
        onCheckout={(item) => handleOpenDirectCheckout(item.product, item.color, item.size)}
      />
